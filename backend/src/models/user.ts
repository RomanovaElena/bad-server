...
const userSchema = new mongoose.Schema<IUser, IUserModel, IUserMethods>(
    ...
    },
    {
        ...
        toJSON: {
            virtuals: true,
            transform: (_doc, ret) => {
                // Чтобы TS не ругался, приводим поля к optional через '?'
                if ('tokens' in ret) delete (ret as Partial<typeof ret>).tokens
                if ('password' in ret) delete (ret as Partial<typeof ret>).password
                if ('_id' in ret) delete (ret as Partial<typeof ret>)._id
                if ('roles' in ret) delete (ret as Partial<typeof ret>).roles
                return ret
            },
        },
    }
...
userSchema.methods.generateAccessToken = function generateAccessToken() {
    ...
    return jwt.sign(
        ...
        ACCESS_TOKEN.secret,
        {
            ...
            subject: (user._id as Types.ObjectId).toString(),
        }
    )
}
